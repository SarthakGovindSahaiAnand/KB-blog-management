...
const DashboardContainer = styled.div`
  display: flex;
  min-height: 100vh;
  background-color: #f0f2f5; /* Lighter background for the overall dashboard */
`;
...
const SidebarItem = styled.div`
  ...
  background-color: ${props => props.active ? '#e9ecef' : 'transparent'}; /* Light gray for active */
...
const ContentArea = styled.div`
  flex: 1;
  padding: 0; /* Adjust padding here, removed from MainContent */
  display: flex;
  flex-direction: column;
`;
...
const DropdownContainer = styled.div`
  display: flex;
  flex-direction: row; /* Horizontal layout for dropdowns */
  gap: 10px;
...
const CheckboxContainer = styled.div`
  ...
  border-radius: 5px;
  background-color: white;
  max-height: 150px; /* Limit height to prevent excessive scrolling */
  overflow-y: auto;
  min-width: 180px;
`;
...
// New styled components for dropdown access control
...
const ItemCard = styled.div`
  ...
  background-color: #ffffff;
  box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05); /* Enhanced shadow */
  display: flex;
  justify-content: space-between;
  align-items: center;
...
const SuperAdminDashboard = () => {
  const { 
    ...
    logout, 
    deleteUser,
    toggleManageAllBlogsAccess, // Added new function
    listAdmins,
  ...
  const [blogAccessMap, setBlogAccessMap] = useState({}); // Stores blogId -> [adminUids]
  
  // New state for dropdown management
  const [openDropdowns, setOpenDropdowns] = useState({}); // Track which dropdowns are open
  const dropdownRefs = useRef({}); // Track dropdown refs for outside click detection
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [allBlogs, allUsers, allAdmins] = await Promise.all([
          listBlogs(),
          listUsers(),
          listAdmins(),
        ]);
  ...
  }, [currentUser, listBlogs, listUsers, listAdmins]);
    ...
    const fetchBlogAccess = async () => {
      ...
      for (const blog of blogs) {
        try {
          ...
          // Optionally set an error for this specific blog or just log it
        }
      }
  ...
  const handleToggleBlogAccess = async (userUid, currentAccess) => {
    try {
      clearMessages();
      const newAccess = currentAccess === 1 ? 0 : 1; // Toggle access
  ...
  const handleDropdownCheckboxChange = (blogId, adminUid, isChecked) => {
    ...
    handleBlogSpecificAccessChange(blogId, newAdminList);
  };
          ...
          {activeSection === 'users' && (
            <>
              <ContentHeader>
                ...
                <AdminSection>
                  {/* Existing dropdowns for role and status management */}
                  <DropdownContainer>
                    <Select 
                      value={selectedUser} 
                      onChange={(e) => {
